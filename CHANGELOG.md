...
- Added `DispGraph`, which clears the pixel buffer and then resamples every *enabled* `Y1`-`Y9`/`Y0` function across the window and plots each result, sampling pixel columns 0-94 at a stride of `Xres` (`x_step = ((Xmax - Xmin) / 94) * Xres`); disabled and undefined slots draw nothing. Clearing first means a redraw after changing Y= definitions or window variables never leaves stale pixels behind, matching a full graph redraw on real hardware
...
### Fixed
- Fixed the string-store shorthand `"<text>→<var>` (unclosed quote) tokenizing the `→` and destination as part of the string literal instead of storing: `→` (STO) now terminates an *unclosed* string, so `"HELLO→Str1` stores `HELLO`, matching real hardware's optional closing quote. The shorthand only applies when the literal is actually unclosed — a `→` inside a properly closed string (e.g. `Disp "A→B"` or `"A→B"→Str1`) is preserved as a literal character
